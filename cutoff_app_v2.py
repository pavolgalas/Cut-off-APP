...
st.markdown("""
<style>
.sidebar .slider .stSlider > div > div > div > div {flex-direction: row-reverse;}
</style>
...
def cutoff_row(label, min_val, max_val, df_col):
    """Slider + number input in compact row"""
    st.markdown(f"**{label}**")
    col1, col2 = st.columns([3,1])
    with col1:
        slider_val = st.slider("", min_value=min_val, max_value=max_val,
                             value=safe_default(df_col, 0.75, (min_val+max_val)/2),
                             step=0.5, key=f"slider_{label.replace(' ','_')}")
    with col2:
...
def show_data_preview(df, cutoffs):
    st.header("📊 Data Preview (Filtered)")
...
def calculate_metrics(df, cutoffs):
    ...
    for criterion in criteria:
        ...
        metrics[criterion] = {
            ...
            'no_data_pct': 100 * no_data_count / total if total > 0 else 0
        }
    ...
    for _, row in df.iterrows():
        ...
        if applicable:
            ...
            (overall_pass if passes_all else overall_fail) += 1
    
    total_eval = overall_pass + overall_fail
    metrics['overall'] = {
        'pass': overall_pass, 'fail': overall_fail,
        'total_evaluated': total_eval,
        'pass_pct': 100 * overall_pass / total_eval if total_eval > 0 else 0,
        'fail_pct': 100 * overall_fail / total_eval if total_eval > 0 else 0
    }
    return metrics
...
# MAIN APP - PERFECT SIDEBAR LAYOUT RESTORED
st.title("🎯 Project Cutoff Analysis")
...
if uploaded_file is not None:
    ...
    # SIDEBAR: Cutoff Inputs (BOTH SLIDER + NUMBER)
    ...
    cutoffs = {}
    cutoffs['Final report score (Average)'] = cutoff_row(
        "Final Report", 0.0, 100.0, df['Final report score (Average)'])
    cutoffs['Absorption rate (Average)'] = cutoff_row(
        "Absorption", 0.0, 100.0, df['Absorption rate (Average)'])
    cutoffs['Progress report score'] = cutoff_row(
        "Progress", 0.0, 45.0, df['Progress report score'])
    cutoffs['QS report score'] = cutoff_row(
        "QS", 0.0, 45.0, df['QS report score'])
    ...
    summary_data = [{ 
        'Criterion': k, 'Cutoff': f"{cutoffs[k]:.1f}",
